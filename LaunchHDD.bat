...
echo.
echo.
echo Se ejecutara el HDD virtual desde tu PC usando QEMU.
...
.\qemu\qemu -L .\qemu -kernel-kqemu -full-screen -localtime -soundhw all -m %mem% -hda %_iso_name% -boot c
exit
:run_no
cls
echo. 
echo EL HDD SE ESTA EJECUTANDO.
.\qemu\qemu -L .\qemu -kernel-kqemu -localtime -soundhw all -m %mem% -hda %_iso_name% -boot c
exit